...
function renderOverview(d) {
  ...
  const rows = [
    ...
    ['Template', escapeHtml(d.template?.name || '—')],
    ...
    ['— Call-1 Routing —', ''],
    ['Provider / Model', d.templateSelectLlm ? `${escapeHtml(d.templateSelectLlm.provider)} / ${escapeHtml(d.templateSelectLlm.model)}` : '—'],
    ['Duration', d.templateSelectLlm ? `<span class="debug-badge ${timingClass(d.templateSelectLlm.durationMs)}">${formatMs(d.templateSelectLlm.durationMs)}</span>` : '—'],
    ['TTFT', d.templateSelectLlm?.ttftMs != null ? formatMs(d.templateSelectLlm.ttftMs) : '—'],
    ['— Call-2 Fill —', ''],
