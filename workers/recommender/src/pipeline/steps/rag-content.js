/**
 * RAG Content Step — unified guide + experience + hero image search via Vectorize.
 * Generates one embedding, queries CONTENT_INDEX, splits by metadata type.
 * Writes ctx.rag.guides, ctx.rag.experiences, and ctx.rag.heroImages.
 */
...
export async function ragContent(ctx, config, env) {
  ...
  } = await searchContent(ctx.request.query, env, config);
